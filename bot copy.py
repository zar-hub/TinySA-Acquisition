...
if __name__ == "__main__":
    ...
    # parameters
    filename = "HF_LNA_SIL_R12.txt"
    cols = "Frequency (GHz),Peak (mV),Noise (mV),Time (s),SigPeak (mV),SigNoise (mV)"
    sample_freq = np.linspace(2.2e9, 2.8e9, 6)
    ...
    # peaks and noise
    data = np.zeros((len(sample_freq) * maxk, 3))
    sig = np.zeros((len(sample_freq) * maxk, 2))
    ...
    for k in range(maxk):
        for i, freq in enumerate(sample_freq):
            ...
            print(f"Peak: {peak}, Noise: {noise} , Time: {time}")
            print(f"Peak std: {sig_peak}, Noise std: {sig_noise}")
            
            data[k*6 + i, 0] = peak
            data[k*6 + i, 1] = noise
            data[k*6 + i, 2] = t
            sig[k*6 + i, 0] = sig_peak
            sig[k*6 + i, 1] = sig_noise
            
            
            # update the values of the peak and noise
            ax[1].clear()
            for j in range(k+1):
                ax[1].errorbar(sample_freq/1e9, data[j*6:(j+1)*6,0] * 1e3,
                               yerr = sig[j*6:(j+1)*6,0]* 1e3, fmt = 'g-')
                ax[1].errorbar(sample_freq/1e9, data[j*6:(j+1)*6,1] * 1e3, 
                        yerr = sig[j*6:(j+1)*6,1]* 1e3, fmt = 'r-')
            plt.draw()
        time.sleep(5)
    ...
    freq = []
    for i in range(maxk):
        freq = np.append(freq, sample_freq)
    np.savetxt(filename, np.column_stack((freq * 1e-9, data[:,0:2 ] * 1e3, data[:,2], sig * 1e3)), 
               delimiter=',', 
               header=cols,
               fmt="%.5e")
